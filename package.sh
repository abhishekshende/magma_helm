...
if [[ -z $HELM_CHART_ARTIFACTORY_URL ]]; then
  ...
  if  [ "$DEPLOYMENT_TYPE" == "$ALL" ]; then
    ...
    helm dependency update "$MAGMA_ROOT/dp/cloud/helm/dp/charts/domain-proxy"
    helm package "$MAGMA_ROOT/dp/cloud/helm/dp/charts/domain-proxy" $VERSION && helm repo index .
  fi

  # Push charts
  git add . && git commit -m "orc8r charts commit for version $ORC8R_VERSION"
  git remote add origin2 "$GITHUB_REPO_URL"
  ...
  RESPONSE_CODE_REPO="$(curl --output /dev/null --stderr /dev/null --silent --write-out "%{http_code}" --user "$HELM_CHART_MUSEUM_USERNAME":"$HELM_CHART_MUSEUM_TOKEN" "$HELM_CHART_ARTIFACTORY_URL/$HELM_CHART_MUSEUM_REPO/" || :)"
